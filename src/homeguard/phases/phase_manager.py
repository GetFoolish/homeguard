"""Phase Manager for homeguard rollout system."""
...
class PhaseManager:
    """
    Manages the 6-phase rollout system for homeguard.
    ...
    async def _apply_domain_rule(self, rule: PhaseRule):
        ...
        if rule.action == "block":
            ...
            await self._run_iptables_command([
                "iptables", "-t", "filter", "-I", "HOMEGUARD_FILTER", "10",
                "-p", "udp", "--dport", "53",
            ...
            try:
                ...
                await self._run_iptables_command([
                    "iptables", "-t", "filter", "-I", "HOMEGUARD_FILTER", "11",
                    "-d", ip, "-j", "DROP"
                ], ignore_errors=True)
